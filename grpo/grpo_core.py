"""
GRPO core: rollout generation and policy gradient update - Mac Compatible.
Adapted from https://github.com/policy-gradient/GRPO-Zero
Changes: Device-agnostic cache clearing, autocast handling for CPU/MPS.
"""
import dataclasses
import gc
import math
...
def rollout(
    ...
    for cur_pos in range(min_prompt_len, total_len):
        ...
        # Use autocast only on supported devices
        if device.type == "cuda":
            with torch.autocast(device_type="cuda", dtype=dtype):
                logits = model.inference(tokens[:, prev_pos:cur_pos], prev_pos)
        else:
            # CPU/MPS: run in default dtype (float32)
            logits = model.inference(tokens[:, prev_pos:cur_pos], prev_pos)
...
def update_policy(
    ...
    for i in range(0, len(episodes), micro_batch_size):
        ...
        batch_token_ids = [
            ...
            + [pad_token_id] * (batch_max_length - batch_lengths[bi])
            for bi, episode in enumerate(batch_episodes)
        ]
        batch_masks = [
            [0] * len(episode.prefix_token_ids)
            + [1] * len(episode.generated_token_ids)
            + [0] * (batch_max_length - batch_lengths[bi])
            for bi, episode in enumerate(batch_episodes)
        ]
        ...
        # Forward pass with autocast for CUDA only
        if device.type == "cuda":
            with torch.autocast(device_type="cuda", dtype=dtype):
